...
def synthesize(ssml: str, output: Path, *, api_key: str, folder_id: str,
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if "audio" not in content_type and not response.content.startswith((b"ID3", b"\xff\xfb", b"\xff\xf3", b"\xff\xf2")):
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Генерация аудио урока JEMIX 1.4 в Yandex SpeechKit")
    ...
    parser.add_argument("--slide", type=int, help="Сгенерировать только один слайд, например 7")
    parser.add_argument("--overwrite", action="store_true")
    args = parser.parse_args()

    api_key = get_required_env("YANDEX_API_KEY")
    folder_id = get_required_env("YANDEX_FOLDER_ID")
    ...
    print(f"Голос: {args.voice}; амплуа: {args.emotion}; скорость: {args.speed}")
    for ssml_file in files:
        ...
        synthesize(
            ...
            voice=args.voice, emotion=args.emotion, speed=args.speed,
        )
        print(f"  OK: {output.stat().st_size:,} bytes")

    print("Готово. MP3 находятся в папке audio")
    return 0
